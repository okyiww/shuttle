...
- `SkillLoader(projectRoot)`：`.shuttle/skills`（rank 高，近处优先）+ `~/.shuttle/skills`（rank 低），只扫一层，`<name>/SKILL.md` 或 `<name>.md` 两种形态；frontmatter 只认 `key: value` 标量行；缺 `name`/`description` 或 name 非 kebab-case → 丢弃并 `console.warn`。`catalog()` 返回 `{name, description, digest}`；`get(name)` 返回完整文件内容（含 frontmatter）。每次调用重扫，无 watch、无缓存。
